...
- **Audio: pull-based AudioWorklet, not push-scheduling**: `app/lib/dos-emulator.ts` posts raw `Float32Array` chunks via `port.postMessage(chunk, [chunk.buffer])` (transferable). The worklet (`public/dos-audio-processor.js`) owns a ring queue (6144 sample cap, 2048 prime threshold) and drains it inside `process()` at the audio thread's natural rate. **Do not** revive the old `createBufferSource + start(t)` push-scheduling pattern with a `MAX_LEAD` cap — that worked on desktop only because of low main-thread jitter and silently dropped nearly all chunks on mobile.
...
- **Audio diagnostic badge** (`onAudioStatus` callback): plumbed through `DosEmulator → DosFrame → _index.tsx`, rendered as a top-right scrolling log of the last 15 status lines with `+ms` timestamps. Auto-hides 5 s after the first `play …` tick; stays visible if any message contains an error keyword (`err|fail|stuck|rejected|timeout`). Keep this when touching audio code — it's how mobile-only failure modes get diagnosed without a Mac↔iPhone remote inspector setup.
