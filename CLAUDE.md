...
- **Audio scheduler `MAX_LEAD = 0.01` (~10 ms)**: At boot the WASM worker batches audio chunks while the main thread is busy, then drains them. Each chunk advances `nextAudioTime` by its full duration, so without the cap the schedule cursor races ahead of `currentTime` and steady-state lag exceeds the AV-sync perception threshold. The cap drops late-batched samples to keep audio aligned with video.
